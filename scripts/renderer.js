$(document).on('click', '.x-menu-container', function() {
  if ($('.crenamed').css('display') == 'none') {
    ...
    var modeloItem = ' <x-doctab id="tab' + currentID + '" selected data-id="' + currentID + '">' +
      ...
      ' <x-label>' + $(this).find('x-label').text() + '</x-label>' +
      ' <span data-id="' + currentID + '" doctab-close>  <x-icon name="close"></x-icon>      </span>'     +
       '</x-doctab>'
...
function addStructToList(q) {
  ...
  q.forEach(element => {
    ...
    elementGet(JSON.stringify(element.params), element.name, element.uid)
...
function initHome(data) {
  ...
  data.forEach(element => {
    console.log(element)
    var label = '<label style=" margin: 0px;  font-size: 11px;    color: #c0c0c094;">ID: ' + element.id + '</label>'
    var item = '<x-menuitem id="container' + element.id + '" container class="x-menu-container" data-id="' + element.id + '">' +
      ' <x-icon name = "dashboard"> </x-icon> ' +
...
function rendererAttr(data) {
  ...
  data.forEach(element => {
    ...
    if (element.visible) {
      ...
      var itemValue = ""
      var item = '<x-box  data-list="' + element.value + '" ' + element.name.trim() + ' data-id="' + element.id + '" id="attribute' + element.id + '" attributes class="x-attributes" data-type="' + element.type + '">' +
        '  <x-label> ' + element.name.trim() + ' </x-label> '

      if (element.editable) {
        itemValue = '  <x-input  xname="' + element.name.trim() + '" bx="' + element.type + '"  id="attr' + element.id + '" class="box-values type-' + element.type.toLowerCase() + '" placeholder="' + element.hint + '" value="' + element.value + '">    </x-input> '
      } else {
        itemValue = '  <x-input  xname="' + element.name.trim() + '"  ' + element.name + ' bx="' + element.type + '"  id="attr' + element.id + '" class="box-values type-' + element.type.toLowerCase() + '" disabled="true" placeholder="' + element.hint + '" value="' + element.value + '">    </x-input> '
      }
      ...
      if (element.type == "COLOR") {
        ...
        if (element.editable) {
          itemValue = '  <x-numberinput bx="' + element.type + '"  suffix="'+suffix+'"  style="cursor:pointer" id="attr' + element.id + '" class="el-'+element.el.toLowerCase()+'  box-values type-' + element.type.toLowerCase() + '" placeholder="' + element.hint + '" value="' + element.response + '">    </x-input> '
        } else {
          itemValue = '  <x-numberinput bx="' + element.type + '"  suffix="'+suffix+'"  style="cursor:pointer" id="attr' + element.id + '" class="el-'+element.el.toLowerCase()+'  box-values type-' + element.type.toLowerCase() + '" disabled="true" placeholder="' + element.hint + '" value="' + element.response + '">    </x-input> '
        }
        ...
        if (element.editable) {
          itemValue = '  <x-input  data-list="' + element.value + '" bx="' + element.type + '"  style="cursor:pointer" id="attr' + element.id + '" class="box-values type-' + element.type.toLowerCase() + '" placeholder="' + element.hint + '" value="' + element.response + '">    </x-input> '
        } else {
          itemValue = '  <x-input   data-list="' + element.value + '" bx="' + element.type + '"  style="cursor:pointer" id="attr' + element.id + '" class="box-values type-' + element.type.toLowerCase() + '" disabled="true" placeholder="' + element.hint + '" value="' + element.response + '">    </x-input> '
        }
