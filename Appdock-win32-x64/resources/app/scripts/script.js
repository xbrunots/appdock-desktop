...
function createObject(object, dock) {
    console.log($(object).attr('itemid'))
    if ($('x-body').find('x-button[create-bottomsheet]').length > 0 && $(object).attr('itemid').indexOf("BOTTOMSHEET") != -1) {
...
function autoCompleteCodes(query) {
...
$(document).on('click', 'div[code-methods]>x-input>x-icon', function() {
    if (parseInt($('div[code-methods]').css('right')) != 0) {
        $('div[code-methods]').css('width', '230px')
        $('div[code-methods]').animate({
            right: '0px'
        })
        $('div[code-methods]').promise().done(function() {
            $('div[code-methods]').animate({
                height: '100%'
            })
            $('div[code-methods]>x-input>x-icon').animate({
                'margin-left': '0px'
            })
            $('div[code-methods]').promise().done(function() {
                $('div[code-methods]>x-input>x-icon').attr('name', 'close')
            })
        })
        ...
        $('div[code-methods]').promise().done(function() {
            ...
            $('div[code-methods]').promise().done(function() {
                ...
                $('div[code-methods]').promise().done(function() {
                    $('div[code-methods]>x-input>x-icon').attr('name', 'search')
                })
            })
        })
    }
...
$(document).on('click', 'x-menuitem[menu-bar-services-icon], x-button[menubar-services]', function() {
    if (parseInt($('x-events').css('left')) < 0) {
        $('x-events').animate({
            left: '450px'
        }, 100)
        $(this).addClass("menubar-selected")
    } else {
        $('x-events').animate({
            left: '-450px'
        }, 100)
        $('x-button[menubar-services]').removeClass("menubar-selected")
    }
})